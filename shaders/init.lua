supTEST = love.graphics.isSupported

scale = 1.0

function love.graphics.isSupported(s) --Can't believe I had to do this...
	local ok, result = pcall(supTest, s)
	if ok then
		return supTest(s)
	else
		return false
...
local supported = true--love.graphics.isSupported and love.graphics.isSupported("canvas") and love.graphics.isSupported("shader") or false
